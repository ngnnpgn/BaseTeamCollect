...
turtles-own [ energy               ;; l'energie de l'agent
              ...
              mem0 mem1 mem2 mem3 mem4 mem5  ;; six zones memoires a utiliser pour la programmation des agents
                                             ;; chacune peut contenir un scalaire ou une liste de 2 valeurs
              ennemy               ;; la couleur de l'ennemi
            ]
RocketLaunchers-own [ nb-missiles max-missiles waiting is-carried? ]
Explorers-own [ carry]
Bases-own [ nb-missiles max-missiles waiting ]
Missiles-own [ range ]
Perceptions-own [ agt range ]
globals [ deplacements victoire energy_green energy_red ]
...
to setup [ config ]
  ...
  repeat 3 [ new-burgers 100 ]

  ; pour toutes les bases
  ask Bases [
  ]

  ; met à jour l'affichage des ressources globales des 2 équipes
...
to go
  ...
  if (random 500 = 0) [ new-burgers 100 ]
...
to update_energy_watches
  ...
  sum [nb-missiles * 10] of RocketLaunchers with [ color = red ] +
  sum [nb-missiles * 10] of bases with [ color = red ]
  ...
  sum [nb-missiles * 10] of RocketLaunchers with [ color = green ] +
  sum [nb-missiles * 10] of bases with [ color = green ]
end
...
to new-burgers [ n ]
  ...
  [
      ...
      fd random 3
      ; energie entre 50 et 150
      set energy 50 + random 100
  ]

  ;; crée un tas en symétrique...
  create-Burgers n
  [
      setxy -1 * x -1 * y
      set heading random 360
...
to new-Explorer [ n a ]
  ...
  if ([energy] of a > n * 1500) [
    ...
    ask a [ set energy energy - n * 1500 ]
  ]
end
  ...
  if ([energy] of a > n * 6000) [
    ...
    repeat n [
      ...
      hatch-RocketLaunchers 1 [
        ...
        ; initialisation de l'energie, du rayon de perception, de la vitesse
        set energy 4000
        set detection-range 5
        set speed 0.5
        ; initialement 30 missiles, 100 au maximum
        set nb-missiles 30
        set max-missiles 100
        set waiting 0
        set my-bases Bases with [ color = [color] of a ]
        set label ""
        set mem0 0
    ...
    ; decremente l'energie de l'agent createur
    ask a [ set energy energy - n * 6000 ]
  ]
end
...
to new-Harvester [ n a ]
  ...
  if ([energy] of a > n * 3000) [
    ...
    ; decremente l'energie de l'agent createur
    ask a [ set energy energy - n * 3000 ]
  ]
end
...
;
to take [ b ]
  ;; seuls les agents de type "moissonneurs" peuvent récolter la nourriture
  if (breed = Harvesters) [
    ; si on est suffisamment proche
    if (distance b <= 2) [
      without-interruption [
...
to forward-move [ d ]
  without-interruption [
    ; s'il n'y a pas d'agents devant (hormis les burgers et les agents de perception)
    if count turtles in-cone d 90 with [ breed != Burgers and breed != Perceptions and breed != Missiles ] = 1 [
      ; si l'agent ne s'est pas deja deplace au cours de ce cycle
      if (not member? self deplacements)
          ; il peut avancer...
        [ ifelse (d > speed)
          [ fd speed ][ fd d ]
          ; ... et il s'ajoute dans la liste des agents qui se sont deplaces
          set deplacements fput self deplacements
        ]
    ]
  ]
end
...
to go-missile
  ...
  let t turtles in-cone 1.5 180 with [ (breed != Missiles) and (breed != Burgers) and (breed != Perceptions) ]
  ; s'il y a des agents
  if any? t [
    ; on decremente leur energie de 100 si ce n'est pas une base
    ask t [ if (breed != Bases) [ set energy energy - 100 ] ]
...
to new-missile [ n ]
  ...
  if (nb-missiles + n <= max-missiles) and (energy > 10 * n) [
    ; le nb de missiles est incremente de 'n'
    set nb-missiles nb-missiles + n
    ; chaque missile coûte 50 points d'energie a fabriquer
    set energy energy - 10 * n
  ]
end
...
to-report perceive-robots [ c ]
  report turtles in-radius detection-range with [ (color = c) and ((breed = Explorers) or (breed = RocketLaunchers))]
end
...
## VOTRE STRATEGIE COMMENTEE - non : NGANKAM FRANCK

-	ma stratégie est la suivante:

	- Implementé une intelligence collective entre agents et pour cela, les memeoires "mem" seront utilisées pour retrouver la position d'un ennemi ou d'un burger , les taches de chaque agent seront locales c-a-d qu'elles affectent la cible dans un rayon bien limité (contraintes du projet), le probleme rencontré ici est que je n'ai pas pu utiliser les listes comme demandées car cela necessitait de modifier le code principale en initialisant a vide ([]) les listes ce qui est interdit. Je me suis donc limité aux 6 variables memoires et cela a un impact significatif lorsque le nombre d'agents est petit car il y'a une baisse d'efficité au niveau de la recherche d'ennemi. J'ai essayé de resouddre ce probleme en creant un grand nombre d'agent (voir une cinquantaine)
quand j'ai assez d'energie afin que la solution au probleme se comporte comme la solution de l'algorithme de fournies vu en cours bien qu'ici il n y'a pas de variable d'intensité de formol).

	- Dans ma solution, "mem5" represente toujours la decouverte d'un ennemi, en plus si mem5 = 2 alors s'est une base ennemie trouvé et 1 sinon. cela me permet dans la suite de donner une priorité aux agents ennemi et de modifier mes strategies. Par expemple lorsque je tire sur une base ennemie, mem5 qui vaut 2 n'est pas remi a zero, et donc les coordonnées sont maintenues afin d'alerter les autres, par contre si mem5 vaut 1, elle est remise azero apres le tire.

	- Vu qu'il ne est pas interdit de piquer de la nourriture ou de l'energie a mon ennemi, j'ai ecris des fontions qui le font et je les ai utilisées. Bien qu'elle semble avoir un impact sur mon energie globle, je ne peut toutes fois prouver son efficacité .

	- A chaque creation de mes bases, les positions de mes agents ( Explorers, Harvesters, RocketLaunchers) sont choisies aleatoirement et on l'avantage de couvrir tous le terrain au debut. Le comportement varie tres peu en fonction du nombre initial des agents vis a vis de sa stabilité dans du temps. Le nombre de nourriture qu'un moissoneur peut porter est de 5000 (que j'ai choisi) et cela est tres benéfique car un tas de nourriture vaut un peu plus de 5000 foods, Je le trouve benefique car un harvester a tendance a cherche a cote lorsqu'il a trouvé une nourriture et cela permet aussi a un explorateur de rester pres d'un tas de food augmentant ainsi ses chances d'alerter un moissoneur ou plusieurs.

	- Problemes : difficile de detruire une base ennemi completement, mais peut etre ameliorer par exemple en positionant les lanceurs de rocket directement sur la position des bases ennemies (l'invere de ma position) et tirer ( a faire lorsque j'ai une cinquantainte de lanceurs de rocket par exemple). Cela m'a semblé un peu une "tricherie"
d'ou la non implementation.
...
-	Les Actions des agents: sont expliquées dans le code.
